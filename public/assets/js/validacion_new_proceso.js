function guardar() {
        ...
        if (data.length > 0) {
            switch (data[0]) {
                case "1": // --- todo salio bien
                    titulo = 'Atención'
                    parrafo = "<span class='text-success'>Se registró el Proceso de forma exitosa, actualizando contenido de la página</span><p  align='center'><img src='../../assets/images/wait2.gif' width='50' height='50'></p>"
                    $('#title_modal').html(titulo)
                    $('#content_modal').html(parrafo)
                    $('#myModal').modal()
                    setTimeout(function () {
                    $('#myModal').modal('hide');
                    document.location = '/subcategoria/'
                    }, 5000);
                break;
                case "0": // --- error
                    titulo = 'Atención'
    ...
    titulo = 'Atención'
    parrafo = "<span class='text-danger'>Debe seleccionar una categoría  y sumnistrar un nombre para el proceso</span>"
    $('#title_modal').html(titulo)
    $('#content_modal').html(parrafo)
    $('#myModal').modal()    
}
